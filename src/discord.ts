...
const makeEmbed = ({
    ...
    embed.addField("Artist" + (artists.length > 1 ? "s" : ""), artists.map((artist) => artist.name).join(", "), true)
...
export default async (splitUrl, spotify, track, me) => {
    return await new WebhookClient(splitUrl[5], splitUrl[6]).send(makeEmbed(await makeData(track, me, spotify)))
}